    ...
    for cab_idx in range(num_cabs):
        ...
        while ci < len(charm_pool) and placed < budget:
            ...
            if item_qty <= remaining:
                ...
                # Split only at a CIOC-group boundary (multiple of CHARMS_PER_TOWER = 8)
                # so no CIOC group is ever split across two cabinets.
                groups_that_fit = remaining // CHARMS_PER_TOWER
                split_at        = groups_that_fit * CHARMS_PER_TOWER

                if split_at == 0:
                    # Not even one full group of 8 fits — close cabinet and carry over
                    placed = budget
                    continue

                part1          = dict(item); part1['qty'] = split_at
                part2          = dict(item); part2['qty'] = item_qty - split_at
